...
public class FrameCounter {
    ...
    private double computeSmoothFpsFromRecentFrames(long now) {
        List<Long> recent = this.samples.stream().filter(s -> now - s.timestamp <= this.smoothWindow).map(s -> s.deltaNanos).toList();
    ...
    private double computePercentileLow(double percent) {
        ...
        List<Long> deltas = this.samples.stream().map(s -> s.deltaNanos).sorted(Comparator.reverseOrder())  // slowest frames first
                .toList();
