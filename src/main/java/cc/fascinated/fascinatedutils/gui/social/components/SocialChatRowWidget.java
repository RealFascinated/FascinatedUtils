...
public class SocialChatRowWidget {
    ...
    public record Props(
            String displayName,
            String avatarMinecraftUuid,
            String snippet,
            int presenceColor,
            boolean selected,
            boolean showUnreadBadge,
            Runnable onSelect,
            Runnable onCloseChannel
    ) {
    }
    ...
    public static FWidget build(Props props, float width, float rowHeight) {
        ...
        float mouseY = frame.pointerY();
                ...
                Identifier avatarTexture = props.avatarMinecraftUuid() == null || props.avatarMinecraftUuid().isBlank()
                        ? null
                        : AvatarTextureCache.INSTANCE.get(props.avatarMinecraftUuid(), () -> {});
                String first = props.displayName() == null || props.displayName().isBlank()
                        ? "?"
                        : String.valueOf(Character.toUpperCase(props.displayName().charAt(0)));
                if (avatarTexture != null) {
                    ...
                    graphics.drawCenteredText(first, avatarX + AVATAR_SIZE / 2f,
                            avatarY + (AVATAR_SIZE - graphics.getFontCapHeight()) / 2f, 0xFFFFFFFF, false, true);
                }
                ...
                String previewDraw = TextLineLayout.ellipsize(normalizeSnippet(props.snippet()), maxLineWidth,
                        segment -> graphics.measureTextWidth(segment, false));
                graphics.drawText(previewDraw, textX, titleY + graphics.getFontCapHeight() + 3f,
                ...
                if (props.onCloseChannel() != null) {
                    ...
                    boolean closeHovered = mouseX >= closeBtnX && mouseX < closeBtnX + CLOSE_BTN_SIZE
                            && mouseY >= closeBtnY && mouseY < closeBtnY + CLOSE_BTN_SIZE;
                    graphics.fillRoundedRect(closeBtnX, closeBtnY, CLOSE_BTN_SIZE, CLOSE_BTN_SIZE, 4f,
                            closeHovered ? 0xAA5C1F1F : 0x22FFFFFF, RectCornerRoundMask.ALL);
                    graphics.drawCenteredText("\u2715", closeBtnX + CLOSE_BTN_SIZE / 2f,
                            closeBtnY + (CLOSE_BTN_SIZE - graphics.getFontCapHeight()) / 2f,
                            closeHovered ? 0xFFFF5555 : FascinatedGuiTheme.INSTANCE.textMuted(), false, false);
                }
            }
            ...
            public boolean mouseDown(float pointerX, float pointerY, int button) {
                ...
                if (props.onCloseChannel() != null) {
                    ...
                    float closeBtnY = y() + (h() - CLOSE_BTN_SIZE) / 2f;
                    if (pointerX >= closeBtnX && pointerX < closeBtnX + CLOSE_BTN_SIZE
                            && pointerY >= closeBtnY && pointerY < closeBtnY + CLOSE_BTN_SIZE) {
