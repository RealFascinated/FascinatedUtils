...
public class WaypointsListComponent extends UiComponent<WaypointsListComponent.Props> {
    ...
    public UiView render() {
        ...
        List<Waypoint> filtered = currentProps.waypoints().stream().filter(waypoint -> queryLower.isEmpty() || waypoint.getName().toLowerCase(Locale.ROOT).contains(queryLower)).toList();

        List<UiSlot> bodySlots = new ArrayList<>();
        if (filtered.isEmpty()) {
            String emptyText = queryLower.isEmpty() ? Component.translatable("fascinatedutils.waypoints.empty").getString() : Component.translatable("fascinatedutils.waypoints.no_results").getString();
            bodySlots.add(UiSlot.of(Ui.label(emptyText, FascinatedGuiTheme.INSTANCE.textMuted(), false, TextOverflow.VISIBLE, Align.CENTER)));
        }
        else {
            for (Waypoint waypoint : filtered) {
                bodySlots.add(WaypointRowComponent.rowSlot(waypoint.getId().toString(), new WaypointRowComponent.Props(waypoint, () -> currentProps.onEdit().accept(waypoint), () -> currentProps.onToggleVisible().accept(waypoint), () -> currentProps.onRequestDelete().accept(waypoint.getId()), currentProps.dimensionLabelFormatter().format(waypoint.getDimension()))));
            }
        }

        return Ui.scrollTracked(3f, 5f, true, scrollOffset -> currentProps.scrollOffsetRef().setValue(scrollOffset), currentProps.scrollOffsetRef(), bodySlots);
    }
     ...
     */
    public record Props(List<Waypoint> waypoints, String searchQuery, Ref<Float> scrollOffsetRef,
                        Consumer<Waypoint> onEdit, Consumer<Waypoint> onToggleVisible, Consumer<UUID> onRequestDelete,
