...
public class FGlobalHudColorApplyRowWidget extends FWidget {
    ...
    private float[] swatchBounds() {
        ...
        float maxSwatchLeft = Math.max(bodyLeft, applyLeft - APPLY_LABEL_GAP_DESIGN - swatchSize);
    ...
    private float[] applyChipBounds() {
        ...
        float chipWidth = APPLY_CHIP_WIDTH_DESIGN;
        float swatchSize = SWATCH_SIZE_DESIGN;
        float chipHeight = Math.max(swatchSize, SettingsUiMetrics.SHELL_CONTROL_HEIGHT_DESIGN);
